...
import type { GameState, Pot, Card as CardType } from './types'
...
export function startNewRound(state: GameState): GameState {
  ...
  const hasActionablePlayer = attempts < players.length
  let communityCards: CardType[] = []
  let phase = GamePhase.PRE_FLOP
  if (!hasActionablePlayer) {
    const dealt = dealCards(deck, 5)
    communityCards = dealt.card
