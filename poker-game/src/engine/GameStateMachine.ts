...
import { allocatePots, determinePotWinners } from './PotCalculator'
...
export function startNewRound(state: GameState): GameState {
  let deck = createShuffledDeck().getCards()
...
export function settleRound(state: GameState): GameState {
  ...
  const playerCardsMap = new Map(
    playerHands.map(({ player, cards }) => [
      player.id,
      { holeCards: player.holeCards, communityCards: cards.slice(2) },
    ])
  )

  // 逐个底池结算，确保短码边池不会被无资格玩家赢走。
  ...
  const updatedPlayers = players.map(p => {
    const winnings = allocations.get(p.id) || 0
    return winnings > 0 ? { ...p, chips: p.chips + winnings } : p
  })
