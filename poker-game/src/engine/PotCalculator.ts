 ...
 * - A All-in 50，B All-in 100，C 跟注 150 → 主池 150（A/B/C 各出 50），边池 100（B/C 各出 50），边池 50（仅 C 有资格）
 */
export function calculatePots(players: Player[]): PotCalculationResult {
  ...
  const betLevels = Array.from(
    new Set(committedPlayers.map((p) => p.totalBetThisHand))
  ).sort((a, b) => a - b)
  ...
  for (const level of betLevels) {
    ...
    const contributors = committedPlayers.filter(
      (p) => p.totalBetThisHand >= level
    )
    ...
    pots.push({
      amount: layerAmount * contributors.length,
      eligiblePlayerIds,
...
export function allocatePots(
  ...
  for (let i = 0; i < pots.length; i++) {
    ...
    if (remainder > 0 && winners.length > 0) {
      // 奇数筹码分配给庄家后第一位有资格的玩家
      const firstEligibleWinner = pot.eligiblePlayerIds.find((playerId) =>
