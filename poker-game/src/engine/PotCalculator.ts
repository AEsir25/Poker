...
export function calculatePots(players: Player[]): PotCalculationResult {
  ...
  const betLevels = [...new Set(contributors.map((p) => p.totalBetThisHand))]
    .sort((a, b) => a - b)
  ...
  for (const level of betLevels) {
    ...
    const eligiblePlayerIds = layerContributors
      .filter((p) => !p.isFolded)
      .map((p) => p.id)
    const amount = layerAmount * layerContributors.length
...
export function allocatePots(
  ...
  for (let i = 0; i < pots.length; i++) {
    ...
    if (remainder > 0 && winners.length > 0) {
      // 奇数筹码分配给该底池中顺位最靠前的赢家，不能给无赢家资格的玩家。
      const firstEligibleWinner = pot.eligiblePlayerIds.find((id) => winners.includes(id))
