  ...
  immer((set, get) => ({
    ...
    submitAction: (action) => {
      ...
      const newTrackers = new Map(
        Array.from(actionTrackers.entries()).map(([playerId, tracker]) => [
          playerId,
          { ...tracker },
        ])
      )
      ...
      // 处理行动
      const newState = processPlayerAction(gameState, action, newTrackers)
      ...
      // 检查轮次是否结束
      if (isRoundComplete(newState, newTrackers)) {
        // 检查是否所有人都 All-in 或弃牌
    ...
    advanceToNextPhase: () => {
      ...
      resetActionTrackers(newTrackers)
